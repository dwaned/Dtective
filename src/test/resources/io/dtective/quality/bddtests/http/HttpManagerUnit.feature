...
Feature: HTTP Messaging
  ...
  Scenario: Functional Test - Key is found within a JSON Object
    Given I add the following data-store "filePath" "/src/test/resources/io/dtective/quality/bddtests/http"
  ...
  Scenario: Functional Test - Key and value are found within a JSON Object
    ...
    Then data-store "jsonSample" contains key "$.data.items[0].country.title" with anything other than value "hawaii"
